...
cleanup_on_error() {
    ...
    print_error "Installation failed and has been rolled back"
}
...
check_requirements() {
    ...
    if [ "$ARCH" != "x86_64" ] && [ "$ARCH" != "aarch64" ]; then
        ...
        print_error "This application only supports x86_64 and aarch64 architectures"
        exit 1
    fi
...
check_port_conflict() {
    ...
    if command -v netstat >/dev/null 2>&1; then
        if netstat -tlnp | grep -q ":5000 "; then
            ...
            print_error "Process using port 5000: $PROCESS"
            
            # Ask for confirmation to kill the process
            echo
            ...
            if [[ $REPLY =~ ^[Yy]$ ]]; then
                ...
                if [ ! -z "$PID" ] && [ "$PID" != "-" ]; then
                    ...
                    if netstat -tlnp | grep -q ":5000 "; then
                        print_error "Failed to kill process. Please stop it manually and try again"
                        exit 1
                    else
...
test_binary_compatibility() {
    ...
    if timeout 5s ./network_map --help >/dev/null 2>&1; then
        ...
        if echo "$ERROR_OUTPUT" | grep -q "GLIBC"; then
            ...
            print_error "Binary requires newer GLIBC than available on this system"
            print_error "Please use a system with newer GLIBC or build from source"
            exit 1
        else
            # If it's not GLIBC issue, assume it will work
...
configure_firewall() {
    ...
    if command -v ufw >/dev/null 2>&1; then
        ...
        print_success "Firewall configuration skipped (no supported firewall found)"
    fi
}
    ...
    if systemctl is-active --quiet network-map; then
        ...
        while [ $attempt -le $max_attempts ]; do
            ...
            if [ $attempt -eq $max_attempts ]; then
                ...
                print_error "Check logs with: journalctl -u network-map -f"
                exit 1
            else
                sleep 2
                attempt=$((attempt + 1))
...
cleanup_installer_files() {
    print_step "Cleaning up installer files..."
    
    # Get the current directory where installer was run
    INSTALLER_DIR=$(pwd)
    
    # Remove installer script (this script itself)
    if [ -f "$INSTALLER_DIR/install.sh" ]; then
        rm -f "$INSTALLER_DIR/install.sh"
        print_success "Removed install.sh"
    fi
    
    # Remove downloaded binary (since it's now installed in /opt/network_map/)
    if [ -f "$INSTALLER_DIR/network_map" ]; then
        rm -f "$INSTALLER_DIR/network_map"
        print_success "Removed network_map binary"
    fi
    
    # Remove any temporary files that might have been created
    rm -f /tmp/init_db.py 2>/dev/null || true
...
uninstall() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        print_success "Application directory kept"
    fi
